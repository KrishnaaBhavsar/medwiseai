...
export default function Chatbot({ className }: ChatbotProps) {
  ...
  useEffect(() => {
    const savedSessionId = localStorage.getItem('medwise_chat_session');
    if (savedSessionId) {
  ...
  const loadChatHistory = async (sid: string) => {
    try {
      setIsLoading(true);
      const response = await fetch(`/api/chatbot/history/${sid}`);
      const data = await response.json();
      if (data.success && data.data.messages) {
        setMessages(data.data.messages);
      } else {
        // If history fails or session expired, start new one
        startChatSession();
      }
    } catch (error) {
  ...
  const startChatSession = async () => {
    try {
      ...
      if (data.success) {
        ...
        setSessionId(sid);
        localStorage.setItem('medwise_chat_session', sid);
  ...
  const sendMessage = async (message: string) => {
    ...
    try {
      // If no session exists yet, try to start one first
      ...
      if (!currentSessionId) {
        ...
        if (data.success) {
          ...
          localStorage.setItem('medwise_chat_session', currentSessionId!);
        }
      }
      ...
      const response = await fetch('/api/chatbot/message', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          sessionId: currentSessionId || 'demo',
          message
        })
      });
    ...
    return (
      <div className={cn("fixed bottom-6 right-6 z-[100]", className)}>
        <Button
          onClick={handleOpen}
          size="lg"
          className="rounded-full h-14 w-14 shadow-lg hover:shadow-xl transition-all duration-300 bg-primary hover:bg-primary/90 relative z-10"
        >
  ...
  return (
    <div className={cn("fixed bottom-6 right-6 z-[100]", className)}>
      <Card className={cn(
        "shadow-2xl transition-all duration-300 overflow-hidden flex flex-col",
        isMinimized ? "w-72 h-14" : (isMaximized ? "w-[500px] h-[600px]" : "w-96 h-[500px]")
      )}>
        {/* Header */}
